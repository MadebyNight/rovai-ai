...
mod tests {
    ...
    mod integration {
        ...
        #[test]
        fn configured_team_can_start_other_runtimes_while_lead_is_active() {
            // Owns the Windows shared-root admission seam: a later member must
            // not need a filesystem mutation while the lead waits for its reply.
            let paths = TestPaths::new("rovai-windows-team-projection");
            let mut database = crate::test_support::fresh_schema_database_fast_at(&paths.data);
            let library = SkillLibraryService::new(paths.library.clone()).unwrap();
            let name = "team-projection";
            write_source(&paths.source, name, "team skill");
            let skill = import_skill(&mut database, &library, &paths.source, name, None);
            library
                .set_group_assignments(
                    &mut database,
            ...
            for (agent, adapter) in [
                ("agent_1", "codex-cli"),
                ("agent_2", "claude-code-cli"),
                ("agent_3", "grok-build"),
            ] {
                database.connection().execute(
                    "INSERT INTO adapter_installation(id, adapter_kind, executable_path, command_name, installation_class, source, auth_scope, enabled, version, created_at, updated_at) VALUES (?1, ?2, ?2, 'test', 'custom', 'custom', 'test', 1, 1, ?3, ?3)",
                    params![agent, adapter, now],
                ).unwrap();
                database.connection().execute(
                    "UPDATE agent_profile SET default_runtime_installation_id = ?1, selected_runtime_adapter_kind = ?2, default_model_selection_json = '{}', default_permission_config_json = '{}' WHERE id = ?1",
                    params![agent, adapter],
                ).unwrap();
                database.connection().execute(
                    "INSERT INTO camp_member(camp_id, agent_id, status, capability_overrides_json, version, joined_at) VALUES ('windows-projection-camp', ?1, 'active', '{}', 1, ?2)",
                    params![agent, now],
                ).unwrap();
            }
            SkillProjectionReconciler
            ...
            for group in [".claude", ".grok"] {
                assert!(
                    paths
                        ...
                        .join("SKILL.md")
                        .is_file(),
                    "configured teammates must be projected before the lead registers"
                );
            }
            assert!(has_active_run_registration(&database, &paths.root, None).unwrap());
            for (agent, adapter) in [
                ("agent_2", AdapterKind::ClaudeCodeCli),
                ("agent_3", AdapterKind::GrokBuild),
            ] {
                database.connection().execute(
                    "INSERT INTO conversation(id, camp_id, agent_id, created_at, updated_at) VALUES (?1, 'windows-projection-camp', ?1, ?2, ?2)",
                    params![agent, now],
                ).unwrap();
                database.connection().execute(
                    "INSERT INTO agent_run(id, camp_id, conversation_id, initial_camp_context_through_sequence, initial_conversation_context_through_sequence, responsibility_key, start_reason, purpose, completion_role, effective_config_json, workspace_json, status, idempotency_key, runtime_adapter_kind, execution_epoch, created_at, started_at, updated_at, invocation_kind, anchor_message_id, current_public_tail_sequence)
                    SELECT ?1, camp_id, ?1, 0, 0, ?1, 'initial', 'test', 'required', '{}', workspace_json, 'running', ?1, ?2, 1, created_at, started_at, updated_at, invocation_kind, anchor_message_id, current_public_tail_sequence FROM agent_run WHERE id = 'windows-projection-run'",
                    params![agent, adapter.as_str()],
                ).unwrap();
                let exposure = SkillProjectionReconciler
                    .prepare_run_exposure(&mut database, &library, agent, &paths.root, adapter)
                    .unwrap();
                assert_eq!(exposure.snapshot.skills[0].status, "ready");
            }
            let registrations: i64 = database
                .connection()
                .query_row(
                    "SELECT COUNT(*) FROM skill_projection_run_registration",
                    [],
                    |row| row.get(0),
                )
                .unwrap();
