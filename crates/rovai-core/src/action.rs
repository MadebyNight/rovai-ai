...
impl ActionSafetyService {
    ...
    /// Rovai-ai received a protocol-level authorization request. This is an
...
mod tests {
    ...
    fn workspace_scope_rejects_parent_traversal() {
        let directory = std::env::temp_dir().join(format!("rovai-action-scope-{}", Uuid::new_v4()));
    ...
    fn runtime_permission_scope_rejects_symlink_escape_and_read_only_write_grant() {
        ...
        let directory =
            std::env::temp_dir().join(format!("rovai-permission-scope-{}", Uuid::new_v4()));
        let workspace_root = directory.join("workspace");
