...
The immutable ContextManifest evidence that freezes one Camp's stable attachment catalog revision, semantic digest, relative payload paths and complete semantic identity for explicitly referenced Published Attachments. It proves append-only successful publication history while excluding absolute roots, device/inode/file identity, publication operation and physical generation. A later attachment-local availability failure does not rewrite a historical receipt, but new Context cannot reference the unavailable path. Current local filesystem integrity and dispatch authorization remain separate Runtime Attachment Auth Receipt concerns, so a semantics-preserving controlled rebuild or current availability change cannot invalidate historical evidence.
...
The stable, read-only path of one currently available Published Attachment payload inside its Camp Published Attachment View. New model input and ContextManifest attachment references obtain it only from the Core resolver; every eligible Agent in that Camp may discover and read it even when the owning message is outside the current frozen Context. When exact integrity cannot be proved, the attachment has no current Published Attachment Path and is omitted rather than blocking the Camp; exact Authority recovery restores the same stable path. It is neither an Authority Attachment path nor permission to enumerate Drafts, another Camp, the View parent, or a user-selected Project workspace.
