...
describe('Channel settings', () => {
  ...
  it('opens DingTalk as a selectable provider and exposes its saved management facts', () => {
    ...
    expect(unnamed).toContain('登录态待校验')
    expect(unnamed).toContain('连接状态未知')
    expect(unnamed).not.toContain('连接在线')
  ...
  it.each(['not_connected', 'session_expired', 'connected'] as const)(
    ...
    (status) => {
      ...
      expect(markup).not.toContain('aria-disabled="true"')
      expect(markup).toContain('钉钉连接')
      expect(markup).toContain(status === 'connected' ? '管理连接（钉钉）' : status === 'not_connected' ? '连接钉钉' : '重新连接')
      expect(markup.includes('星海科技')).toBe(status !== 'not_connected')
      if (status === 'session_expired') expect(markup).toContain('登录已失效')
  ...
  it('renders connected account and published Bot facts without exposing credentials', () => {
    ...
    expect(markup).toContain('aria-label="管理连接（飞书）"')
    expect(markup).toContain('aria-haspopup="menu"')
    expect(markup).toContain('aria-expanded="false"')
    expect(markup).not.toContain('>切换账号</button>')
    expect(markup).not.toContain('>断开</button>')
