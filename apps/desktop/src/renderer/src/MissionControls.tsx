...
export function Icon({ name }: { name: 'board' | 'list' | 'plus' | 'chevron' | 'chevron-right' | 'play' | 'branch' | 'history' | 'more' | 'check' | 'expand' | 'tag' | 'refresh' }) {
...
export function MissionContextMenu({ m, position, catalog, onClose, onStatus, onLead, onSaveTags, onDelete }: {
  ...
  catalog: string[]; onLead(id: string): void; onSaveTags(tags: string[]): Promise<void>; onDelete(): void
}) {
  const person=usePeople();
