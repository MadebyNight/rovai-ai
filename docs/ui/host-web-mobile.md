...
第四稿继承前稿收敛，压缩会话列表、移除日期与聚焦装饰，并对齐 Desktop 的连续 Run、任务字段和设置图标。
前轮“远程连接”分组已同步修改共享生产导航；本轮只调整 Mobile 交互稿。
...
使用既有语义 token。此处不改变 Desktop 主题。手机根页面的新建入口使用无底色圆圈加号，项目级使用普通
加号，均保留 44px 触摸区域；弹窗内“新建”等提交按钮继续使用既有会话主操作 token。

根页面使用五个带文字的底部入口：对话、队员、记忆、定时、设置。详情页一个返回按钮，返回恢复阅读位置；
普通页面顶部只显示页面标题和操作，没有重复的 logo、品牌行或 slogan；登录与关于保留必要品牌识别。
设置没有桌面的折叠、后退、前进三按钮。进入对话后移除全局底栏，把空间留给输入。
对话内通过“对话 / 执行 / 任务”切换，不把 Tab 切换压入返回历史；标题返回始终退出到对话列表。
项目打开和搜索收为标题旁的小图标，搜索点开后才显示输入框；会话行只保留较小标题与蓝色新动态点，
连续排布、无行间分隔线和额外空行，行高 44px。不显示列表日期和“今天 / 昨天”分组；消息保留自身时间。
设置齿轮直接对照生产 `NavigationIcon`，不再使用前稿不规则轮廓。

项目图标采用正式 CampNavigation 的圆角文件夹开合轮廓；对照过 [Lucide Folder Open](https://lucide.dev/icons/folder-open)
与 [Circle Plus](https://lucide.dev/icons/circle-plus) 的线性图标形态，未引入图标依赖。稿中 Rovai 项目放入 11 个
非置顶会话，另有个人笔记与快速对话，可核对不同长度的分组。沿用当前导航的 5 个初始条目及每次增加 5 个：
“查看更多”追加，“收起”恢复 5 个并把阅读位置带回该文件夹；文件夹本身还可整体开合。分组状态独立，
打开会话后返回与刷新保留状态；搜索显示完整匹配，不受折叠与分页截断。

发送区与保存操作位于底部；拍照、照片图库和文件选择分别可达，文件添加面板没有“引用当前附件”。
不设置独立接收者按钮和发送旁的更多菜单；文件右侧提供 @ 入口，默认接收提示沿用生产 Composer。
用户消息靠右，队员消息靠左；队员名后显示 Runtime 名称，例如“爱丽丝 · Codex CLI”，与 WebUI 对齐。
消息菜单保留显式“引用回复”，不模拟手机系统选区的部分引用。
HTML 先展示可交互内容，源码作为次级
视图。消息操作显式可点，不依赖悬停或长按。产品触摸目标至少 44px，输入 16px，处理 safe area 与可视
视口变化。按用户要求和 DESIGN，移除输入黑框、焦点描边和光晕，保留 DOM 焦点、输入、键盘与弹窗操作；
控件本身的静态边界和选中状态继续可辨。支持减少动效。真实软键盘、系统相机和后台恢复仍需设备验收。
...
| 执行与审批 | 自动换行头像、每队员连续 Run、逐 Run/组/命令独立展开、运行高亮、Compact、输出与停止；审批在对话页 | ExecutionAvatarRail 的头像语义、ExecutionStatusGlyph、ExecutionToolGroup、RunningText、执行窗口、审批 Dock |
| 任务 | 新建只有标题、说明、有序验收条件与负责人；详情分别呈现，编辑才有状态 | TaskFields 的 showStatus / requireAssignee 与现有 Task 列表详情 |
...
| 定时任务 | 开关、指令、时间、时区、Cron、单一执行队员、目录与结果；带头像单选，修改与刷新保留 | AutomationWorkspace、AutomationEditor |
...
定时任务对照 `AutomationEditor` 的 `draft.memberId`，每个定义只选择一位执行队员，列表展示该队员头像与名称；
前稿的“执行队伍”及多选页属于错误呈现，第三稿已移除，未增加新的多人定时执行能力。

新建任务对照 `TaskFields` 的 `showStatus={mode === 'edit'}`：创建后为待处理，不让用户在创建时指定状态。
说明和验收条件分别输入、保存、展示；验收条件每行一项、最多 12 项，负责人从当前会话队员选择。
空标题、缺负责人或超出条目上限保留输入并提示；编辑继续保留既有状态项。
...
头像采用 Desktop 浮层的紧凑头像、状态角标与中性选中背景；手机改为自动多行，不保留横向滑条或翻页箭头。
每位队员展示连续 Run 时间线，完成的 Run 默认收起为一行、当前 Run 默认展开，各自可独立开合；示例包含
三次 Run。停止位于所选队员 Header，仅作用于当前 Run，保留历史与其他队员执行。

运行中示例包含“正文 → 已完成 command 组 → 正文 → 当前 command 组”；读文件、修改文件、终端命令、
组和状态图标对照 `ExecutionToolGroup` / `ExecutionStatusGlyph`。预览工具可推进示例当前命令，图标和高亮
随之切换；Compact 有独立演示入口，不与执行中命令同时冒充活动步骤。

组先展开摘要，再按需展开完整输出。每个 Run 与组的展开状态按会话、队员和 Run 分开保存，切换队员或刷新
保留；展开、收起和异步结果到达后保留 summary 的阅读位置。主动滚动解除锚点，“回到最新”沿用圆形向下图标。
等待权限的执行引导回对话，权限表单不在执行 Tab 打开。稿中延迟只是示例，生产沿用共享阅读锚点与缓存机制。
