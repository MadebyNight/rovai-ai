...
进入会话后由标题左侧打开会话列表抽屉，抽屉内仍能切换五个入口；当前会话及草稿留在抽屉下方。
...
| 设置 | 单列滚动共享设置，分类搜索；Desktop Host 保留全部 11 个分类及实际表单，远程连接位于能力分组、渠道上方 | 默认队伍、Skills 生效组、MCP、Runtime、监控和诊断；Server 按宿主能力隐藏渠道等入口 |
