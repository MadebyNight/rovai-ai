...
对话列表连续排列，44px 行高、14px 标题，无行间分隔、额外空行或“今天 / 昨天”分组；蓝点在标题左侧并与标题垂直居中。
...
置顶与项目标题为不带图标的 15px、600 字重文字；置顶会话保留 20px 对话图标，与项目文件夹对齐，项目不显示数量或额外展开箭头。
...
新建表单以底部弹层显示，标题与操作区固定，主体随可视视口滚动。目录、队员和负责人在独立选择面板中操作；
队员为单列整行触摸，支持全选、取消全选和取消最后一位。空队伍可完成选择并返回，点击「新建」时才在字段下
显示必选错误，不调用 Host 创建；重新选择清除错误并补齐负责人。返回或 Escape 关闭当前选择面板，保留名称、
