...
Desktop 完整管理能力保留。Desktop 托管的 Web 使用同一个正式页面，说明：
“渠道由运行此服务的 Rovai Desktop 管理。连接、切换账号和重新登录，请在该电脑的桌面应用中完成；已有账号的 Bot 发布和重试可以在此操作。”
该 Web 不显示原生账号操作按钮和登录 QR，也不展示本机执行台配置。账号摘要、队员 Bot、发布进度、失败恢复、
审批人下拉选择继续共用；仅真实需要重新登录或原生页面的步骤引导回宿主 Desktop，不能用网络失败替代登录失效。
既有结构化审批候选项直接在 Web 表单选择并提交。

独立 Server 在正常设置导航隐藏渠道；遗留入口说明“独立 Server 当前不支持飞书／钉钉渠道。渠道功能请使用 Rovai Desktop。”
不暗示两个独立实例共享配置。部署能力由认证 Host 返回，不通过是否存在 Electron 全局对象猜测。

队员行分别显示持久发布事实和实时连接状态；后续失败不抹去已有发布事实。没有运行期观察时显示连接状态未知。
Web 页面定时授权读取进度，读取失败保留原发布记录并将实时信息标为未知。切页、关闭浏览器、退出 Web 会话不取消发布。
...
飞书和钉钉账号名旁显示最近一次运行期登录校验：登录有效、登录已失效、暂不可用或待校验；数据库存在账号不等于登录校验通过。连接行右侧只保留“管理连接”文字按钮与展开箭头。
