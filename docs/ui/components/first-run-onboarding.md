...
页头显示只读的“1 / 3、2 / 3、3 / 3”进度及可访问的完整步骤提示，不提供跨步骤跳转。欢迎页品牌图形
使用标准星芒、地平线与交汇点几何；为原生窗口控制预留的页头区域保持不变。
...
运行时页左侧以小头像和姓名横排呈现，不再使用带边框的纵向队员卡片或重复加入说明；模型表单使用分隔线
下的开放布局。常用、可选、当前选中和实验性运行时保持直接可见，其余条目置于“其他运行时”折叠区，
保留完整目录与真实资格状态。重新扫描位于运行时面板标题右侧。
...
空结果的图标、标题、说明和操作共用左侧基线；扫描失败时将“重新扫描”主操作放在“查看安装引导”之前。
...
选择卡片只替换当前 Active Camp 的 Desktop-local 输入、聚焦并把光标放在末尾；切换、刷新或重启按普通
Camp Composer 规则恢复。仅辅助技术可见的 live status 说明“内容已填入，可编辑后发送。”。
