...
每次根页面导航重新建立独立的 12 秒期限；同页重复握手、内部 iframe 加载不重置它。未收到确认时显示
“无法确认页面状态”并提供重试，保留页面；不能把 iframe load 或 HTTP 错误响应当作“已加载”。
文档本身明确失败才显示完整失败页，不无限转圈或自动刷新。新页不展示上一页的资源错误，未知身份期间也不把旧诊断冒充当前页问题。
