...
Mission 的活动是同一标签集合中的非文件 Tab，展示交付与使命历史，复用选择、关闭、键盘、阅读位置及
最后一个标签关闭后收起的行为，不申请文件 handle。使命顶栏跨越整个会话，标签栏放在预览正文上方；
普通会话仍保留共享顶栏结构。活动入口的切换和抽屉行为见[使命板](mission-board.md)。
...
- HTML 默认交互预览。Desktop 在不同源 HTTP(S) iframe 中执行，采用独立的 32 MiB 网页上限；超过网页上限使用分页原文。Web 复用同一 Viewer、源码切换、查找与诊断，使用无 `allow-same-origin` 的 opaque sandbox，保留 Host 的 20 MiB 文件读取上限；当前支持单文件 HTML 和 HTTP(S) 依赖，本地多文件站点依赖尚未接通。初始化失败明确显示失败和重试，源码查看读取未注入原稿；
