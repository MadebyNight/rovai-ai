...
Mission 活动是窄预览例外：默认 320px、稳定最小宽度 300px，与 420px 的会话区共同决定其分栏阈值；
普通文件仍使用上面的 420px 最小宽度和稳定比例。用户缩窄 Mission 抽屉并跨过当前内容的分栏阈值时，
父级先收起预览、保留所有 Tab 与阅读位置，让消息区继续可见；这次自动收起不禁止用户随后通过最右侧按钮
显式打开紧凑预览。
