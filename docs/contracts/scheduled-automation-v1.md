...
更新定义时，Core 比较规范化后的新旧 schedule。只有 schedule 实际变化，或定义从关闭变为开启，才重新计算
`nextRunAt`；只修改名称、Prompt、队员、项目或通知渠道必须保留原值，不得静默跨过尚未领取的 occurrence。
...
恢复边界只由应用启动或 Desktop 明确的系统恢复事件推进。应用持续运行时，普通调度 tick 即使晚到数秒，也按原
`scheduledFor` 正常领取，不能按 missed 跳过。Cron 校验、下一次与最近一次 occurrence 都使用同一套五段求值器。
...
每轮结算检查全部 `running | cancelling` AutomationRun，不能让较早的长期运行永久遮挡后续已终态 CampTurn。
...
`enabled` 只控制自动计划领取。显式 `run` 可以执行关闭的定义，包括已经消费并关闭的 once；它创建新的 manual
AutomationRun，不改变 `enabled` 或 `nextRunAt`，也不消费计划 occurrence。已有活跃运行时仍按 overlap 规则记录跳过。
...
- [Built-in Tool Transport v23](builtin-tool-transport-v23.md)
