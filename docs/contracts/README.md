...
| [Camp History Retrieval v5（当前）](camp-history-v5.md) | v3 读取/授权/身份语义不变；CLI 在 Schema 前把省略 mode 安全补全为 timeline/before/20，消息锚点模式仍显式 |
...
| [Built-in Tool Transport v25（当前）](builtin-tool-transport-v25.md) | 新增三个当前 Camp Mission 操作；共 26 项；不暴露业务版本或工作区 |
| [Built-in Tool Transport v24（historical）](builtin-tool-transport-v24.md) | v22 transport 与 Single Chat history 不变；新增七项 Scheduled Automation 操作，catalog 扩为二十三项 |
...
| [ContextManifest Evidence v24（当前）](context-manifest-evidence-v24.md) | 使命事实、可信开始输入、独立 WORKSPACE 与 native binding 接收证据；冻结 22/23 保留 |
| [ContextManifest Evidence v23（historical）](context-manifest-evidence-v23.md) | v21 selection/evidence 不变；Formatter/Manifest 22 增加 ExternalPrincipal direct source 与 ExternalQuote deterministic projection |
