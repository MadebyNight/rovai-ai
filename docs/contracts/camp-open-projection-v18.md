...
Renderer 按详情可视高度估算页大小（12–48 项），首屏后只预取相邻更早一页。用户向上滚到边界或点击
“加载更早记录”才翻页；预取成功不递归预取、不挂载内容。最多挂载两页逻辑项，单条完整正文的像素高度不固定。

页面数据与 DOM 使用不同预算：同一打开的 Run 按 cursor 保留已读页，LRU 目标为 12 页、8 MiB 序列化 UTF-16
文本估算量。最新页、当前两页、相邻预取页和历史阅读开始时的最新页优先保留；只有这些必需页本身超过预算时
才允许越过目标，不继续保留其他页。访问过的 cursor 可独立保留。正文 Blob 另有 64 条、8 MiB UTF-16 文本的
LRU 目标，当前展示和最近已读最新窗口的完整正文优先保留。预算不代表精确 JS heap 字节；关闭 Run 卸载这些缓存。

向前翻页保留当前阅读锚点；向下滚动自动恢复已读页，只有缓存淘汰后才重新读取，不提供“加载较新记录”按钮。
“回到最新”复用缓存并跳转，不能仅因跳转重读最新页。加载入口复用会话区的文字箭头、计数和原位状态样式，
计数仅表示已显示项。失败保留已有内容，原位重试同一方向。
Camp/Run 切换、卸载、请求 generation 变化时丢弃旧响应；目标、cursor 或页内顺序不兼容的响应不应用。
首次展开正在执行的 Run 时，跟随意图保留到首个异步页面及完整正文到达，才完成最新位置定位。
运行中 invalidation 合并后更新最新页缓存；阅读历史时冻结当前 cursor 链和阅读锚点，在途刷新也不得替换历史 DOM。
回到最新或向下读到当前链底部后采用已更新的最新页，再按新边界继续分页；不同首尾边界的页不得混成连续窗口。
