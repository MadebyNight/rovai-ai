---
document_type: user-guide
authority: product-showcase
last_updated: 2026-09-10
---

# Rovai AI 产品展示

Rovai AI 将长期队员、共同会话、任务责任与原生 Agent 执行组织在同一个桌面工作台中。以下界面展示多队员如何围绕实际项目协作，以及队员、自动化、Skills、MCP 和运行时的日常管理。

[下载安装](guides/installation.md) · [操作指南](guides/operations.md) · [系统架构图解](architecture/system-views.md)

## 协作工作台

在共同会话中提出目标，队员通过提及直接交接工作、反馈检查结果。右侧执行台展示各成员的运行状态、工具调用与执行细节，便于在讨论和实际操作之间切换。

![多队员在共同会话中交接研发工作，右侧执行台展示成员的工具调用](assets/product-showcase/workspace.png)

## 任务责任

任务把目标、责任范围与要求、负责人和完成状态保留下来。成员可以在持续讨论中承接、检查与关闭任务，让跨轮次协作有明确的责任和交付入口。

![会话任务列表展示负责人和完成状态](assets/product-showcase/tasks.png)

## 会话内单聊

在共同工作期间，可以与指定队员开启独立单聊，核对理解、讨论方案或补充背景。单聊拥有独立的消息与执行空间，内容不会自动发布到共同会话。

![在共同会话旁与指定队员单独核对方案](assets/product-showcase/single-chat.png)

## 长期队员与会话成员

队员保留自己的名称、团队角色、专业职责、性格与工作准则。运行时、模型及权限另行配置，让长期身份与执行引擎可以分别调整。

![队员工作区展示长期身份、角色信息及独立的运行时配置](assets/product-showcase/member-profile.png)

进入具体会话后，可以邀请队员、查看当前参与者，并为默认承接与协调工作设置队长。不同队员可以使用不同的原生 Agent 运行时参与同一项工作。

![会话成员面板展示队长、参与队员及各自使用的运行时](assets/product-showcase/conversation-members.png)

## 定时自动化

将需要重复开展的工作交给指定队员，按时间安排执行；也可以保留手动触发的任务。Issue／PR 巡检、每周总结与更新说明等模板提供常见工作的起点。

![定时任务工作区展示执行安排、负责队员与可用模板](assets/product-showcase/automations.png)

## Skills 与 MCP

Skills 将讨论、追问、评审等协作方法沉淀为可复用的工作指引。平台支持导入、启停、阅读 Skill 内容与管理生效范围。

![Skills 工作区展示技能目录、内容阅读及生效范围](assets/product-showcase/skills.png)

MCP 为队员接入外部工具与信息来源。统一管理服务配置，并选择使用该能力的队员。

![MCP 工作区展示服务配置与使用队员](assets/product-showcase/mcp.png)

## 运行时管理

集中查看本机 Agent 运行时的安装版本与可用状态，并按需检查可用性。队员可以按工作需要选择已接入的执行引擎。

![运行时目录展示本机已安装的 Agent 产品、版本与可用状态](assets/product-showcase/runtimes.png)

## IM 渠道连接

通过飞书、钉钉连接队员，在常用的沟通渠道中发起和跟进工作。渠道工作区统一管理账号连接、队员 Bot 与发布状态。

![渠道工作区展示飞书与钉钉入口、账号连接和队员 Bot 状态](assets/product-showcase/channels.png)
