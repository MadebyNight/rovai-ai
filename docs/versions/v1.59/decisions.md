...
## V1.59-D08：使命复用 Camp，以首次执行基准保留长期工作区

- 状态：accepted
- 日期：2026-09-15
- 当前权威：[Mission 架构](../../architecture/missions.md)、[Mission v1](../../contracts/mission-v1.md)、[ContextManifest v24](../../contracts/context-manifest-evidence-v24.md)

使命需要跨多轮、恢复和队员切换保持连续，但已有 Camp 已经拥有消息、草稿、成员及执行。因此使命作为
独立业务定义关联唯一 Camp，避免第二套会话生命周期。业务状态由当前成员显式维护，不从 Run 终态推断。

选择首次进入 preparing 才创建持久 worktree，并固定首次 commit 为累计比较基准。保存时创建会产生未执行
工作区，逐轮创建或移动基准则会丢失累计意义并增加恢复成本。临时 index 纳入未跟踪文件，保持真实暂存区；
不维护每轮快照或非 Git 内容基线。删除才清理工作区，独立清理记录承担失败后的恢复。

用户确认 Agent 只接收简短身份/状态与首次成功投递的工作环境；使命读写不暴露版本。采用字段补丁与最后
提交覆盖，接受同字段并发编辑的后写覆盖，避免模型理解版本冲突协议。工作环境描述不增加 Agent 管控。
