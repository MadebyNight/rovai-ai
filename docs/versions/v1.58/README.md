...
本轮局部修正已实现：同一环境的过期历史目录先展示；Core 保留未变模型与参数的权限修改语义；编辑器对明确
目录过期拒绝连续等待刷新并重提一次，保留版本和草稿，使用中文提示。轻量刷新已从 Codex 扩展至其余
Runtime 的现有目录路径，保留必要认证/连接，不重复版本及完整会话行为检查。规则同步在
[现有目录边界](../../architecture/runtime-catalog-boundaries.md#模型目录缓存与执行事实)
与 [Runtime Launch v41](../../contracts/runtime-launch-and-verification-v41.md#目录展示与保存恢复)，不新增缓存或调度系统。
Migration 152 在主线 Runtime 启动设置迁移之后，将本版本投影 schema 从 101 升至 102，增加目录独立成功时间与保存绑定 generation；旧目录沿用
原成功时间，无法证明身份连续的旧绑定仍正常验证。

隔离进程回归覆盖目录请求合并、提交后唤醒、完整检查强度与晚到结果的身份围栏；成员编辑器覆盖连续保存、
失败及参数不支持时保留草稿。首批 Codex 模拟协议单样本完整刷新 359 ms、轻量刷新 90 ms，进程启动 5 → 1；
Electron 过期历史列表首帧回调约 17 ms。耗时日志只记录首次展示、刷新总时间与探测次数。
这些是测试夹具结果，未对真实账号、网络、首次无缓存的真实探测或 packaged App 作性能承诺；日常 App 与数据未改动。

后续扩展复用 Claude 控制初始化、Antigravity `models`、Pi 目录 RPC、ACP Session 目录及 ZCode 原生转换。
本轮 50 项定向 Rust 回归通过（Health 24、Adapter 映射 24、Pi Host 1、目录合并/写回/完整检查强度 1）；
另有 3 项真实 Runtime smoke 未执行。ACP 隔离夹具覆盖 39 个路径/认证/Home/失败组合，完整检查行为不变。
Antigravity 模拟单样本完整探测 1041 ms、轻量目录 41 ms，进程启动 3 → 1；Pi Host 请求 7 → 2，
Grok ACP 请求 5 → 3，Kiro ACP 请求 3 → 2，后两者仍保留必要会话认证/初始化。其他 ACP 省去独立版本进程，
不把握手次数未变写成握手提速。ZCode 复用了原生连接/配置路径并通过证据准入测试，尚未实测其原生端到端刷新。
上述调用次数不计 Core 进程和测试启动器；适配器扩展没有额外持久化字段、调度器、常驻进程或轮询。

与主线 Runtime 启动设置 / 显式检查环境刷新对齐后，目录任务沿用冻结的程序与环境变量配置；写回前持有
现有环境更新锁复核 generation。已有草稿检查隔离测试保持公开状态不变，目录任务仍不能满足完整检查。
隔离 worktree 复验：TypeScript、桌面构建、Clippy、文档门禁通过；Vitest 188 文件 / 1952 项通过，
Node 317 项通过 / 2 项平台跳过，Electron bridge 与成员编辑器 16 场景通过。Rust Core（含 slow-tests）
251 项通过 / 6 项人工 smoke 未运行，CLI 35 项及 slow lib 310 项通过。默认 lib 首次 559 / 560 通过；
唯一失败是旧钉钉迁移夹具未移除新增的 152 收据，补齐来源后该测试定向复验通过。
同一 Codex 模拟夹具复验完整刷新 135 ms、轻量 36 ms，缓存 Core 返回 3 ms、Renderer 首显 17 ms；
样本只用于说明本机夹具差异，不代表真实网络性能。完整 `pnpm test` 被主线已有的导航夹具空格匹配断言
拦截（`if(!...` 与检查器要求的 `if (!...`）；后续 Vitest / Node 阶段已分别执行通过，未修改无关导航文件。
