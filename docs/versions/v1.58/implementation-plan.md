...
导航竞态修复将未完成跳转与已提交记录分开，保留连续游标意图；记忆及队员默认选择按已展示记录修正，
不抢占新导航。资源清理回退不取消新跳转，旧预览内容不能恢复已清理资源。一键创建从请求开始持有导航意图，过期成功只刷新列表、过期失败不弹旧 Dialog；
队员历史回放退出新增和个人资料显示模式，同时保留新队员草稿。回归覆盖慢 push 后退、旧记忆选择与新会话打开交错、
创建成功/失败迟到、正常新建以及队员/同队员栏目回放。
