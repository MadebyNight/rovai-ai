...
官方 ACP 接入已达到 **macOS arm64 与 Windows x64 First-Class / qualified**，两个平台各自闭合 14 个核心能力轴。
平台资格分别绑定独立的 [DSH macOS v2 增量归档](../../../qualification/runtime-platform/macos-arm64-deepseek-harness-v2.json)
与 [DSH Windows v1 归档](../../../qualification/runtime-platform/windows-x64-deepseek-harness-v1.json)；前者引用但不改写
macOS v1 的 14 轴历史验收，后者只声明 Windows 10 x64、本地 NTFS，不沿用其他 Runtime 或平台的摘要。
...
| Built-in rovai CLI | Verified / Implemented | contract-v25 当前 22 个操作、70 条证据；原生终端、三种输入源、精确寻址、Gather、历史/附件、新旧 Run lease fencing、原 Session 续轮 | 共用 bundled CLI 与 private IPC，未走 built-in MCP |
...
| Ready / Version / Platform | Verified / Implemented | CLI >=0.1.5-rc.2 门槛、原生 executable fingerprint、initialize/new/resume 与 catalog 检查；schema 107/closed catalogs/选择器接通 | macOS arm64、Windows x64 各绑定独立 DSH digest；macOS x64、Linux x64 保持 not_qualified |
