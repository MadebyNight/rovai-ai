...
| [Built-in Tool Runtime](builtin-tool-runtime.md) | 十五项普通 Camp operation、一项 Single Chat-only history 与七项 Scheduled Automation operation、`camp.read` 安全 Timeline 默认、single-Camp History target、safe Agent output projection、Core Router、Runtime Fleet、精确 help、Dynamic Context、ContextManifest/ACK 与外部 MCP 的关系 |
