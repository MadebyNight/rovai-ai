...
提取与公屏准入是两层边界：所有已适配结构化图片可以保留，只有 Adapter 从原生事件确认的 Codex
`imageGeneration` 或 Antigravity 已完成 generate-image step 写入闭集 `public_display_source`。

TRAE 与 Copilot 的专用图片结果在各自 Adapter 下显式提取，再交给同一 ACP accumulator；Claude、ACP、
Codex MCP 等其他结构化图片同样可以保存，但来源保持未确认，不进入公屏。不增加全局 rawOutput 路径猜测，
也不根据工具名（包括 `generate_image`）、路径、格式或 MIME 推断生图。Antigravity 的 stream-json 完成事件
不携带图片结果，因此仅在 generate_image 完成时，由该 CLI 子进程专属启动日志取得本机 HTTP 端口，读取精确
conversation/step 的结构化 generatedMedia；最终仍由响应中的 step type/status 和关联字段确认来源。
...
Camp Snapshot/Open 的唯一图片 metadata 查询先过滤 `public_display_source` 闭合集，再执行既有同摘要附件
过滤与 Run/epoch 分组。历史及当前未确认来源的行保留但不进入 Renderer；因此实时刷新、重开和终态无消息兜底
使用同一集合。Main 仍只转发 opaque id 的 Camp-scoped 读取；共享 Gallery 在实际显示前用浏览器 decoder 验证
内容，失败仅该图不可用。没有文件预览面板、Root Grant、文件复制框架或第二套权限 UI。文件路径允许目录外
位置和符号链接；普通文件及真实图片解码是读取链的必要检查。
...
Renderer 在来源 Run 尚未公开发言且未终态时保留已准入图片等待，不把无作者图片提前混入其他队员消息；
公开消息出现后把已准入图片并入该 Run 最后一条公开 Agent 消息的图片区，只有终态仍无公开消息时才释放
独立兜底。未确认来源在进入 Renderer 前已经过滤，不能走任一路径补成节点。
...
该链不读取 Runtime 图片来源标记，收紧自动投影不会改变显式图片附件。图片观察不会调用这条发布链，也不增加
...
[Camp Open Projection v20](../contracts/camp-open-projection-v20.md)、展示见
