...
| `deepseek-harness` | DeepSeek Harness | ACP v1 + official tools/result observer | `fine_grained` | 精确 Session/call 的 tool title/input 映射；shell canonical exit/signal/timeout 决定结果，文件 canonical path 决定 operation；未知工具保留 other，不解析结果文本猜退出 | bootstrap/工具身份/一次消费负向 fixture；复用 ACP 终态和输出 fixture | 0.1.5-rc.2 macOS arm64 的实际结果见 [Parity Matrix](../research/deepseek-harness-runtime/acp-0.1.5-parity.md)；未形成其他平台资格 |
