...
| `deepseek-harness` | DeepSeek Harness | ACP v1 + official tools/result observer | `fine_grained` | 精确 Session/call 映射；bash/pwsh→execute、read/read_image→read、write/edit→对应文件语义、glob/grep→file_search、web_search→web_search、web_fetch→fetch、skill→tool；shell canonical exit/signal/timeout 决定结果，canonical path 决定 operation；write/edit 的完整 before/after 仅转换为标准 ACP terminal Diff；未知工具保留 other | bootstrap/工具身份/一次消费/Before-After/大文件回退 fixture；复用 ACP 终态、Diff 和输出 owner | 0.1.5-rc.2 + MiniMax-M3 真实 read/add/edit/empty matrix 通过；edit `+1/-1`、empty edit `+1/-0`，缺 before 的 add 保持路径级回退；未形成其他平台资格 |
