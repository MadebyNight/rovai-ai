...
| `qoder-cli` | Qoder | ACP v1 | `fine_grained` | 同 ACP 合同；`1.1.28 kind=search + query-only rawInput` 可投影 Web 搜索，`{output_mode,path,pattern}` 内容搜索不得准入；成功 Edit/Write 可从同 ToolCall 先前唯一标准 location 生成文件操作行，terminal kind 冲突不覆盖首次可信 kind | Search Operation positive/negative、path-only、sparse terminal location 与 Read→edit 冲突 fixture 通过 | Skill turn 通过；[`1.1.28` post-fix 文件操作矩阵](../versions/v1.52/runtime-acceptance.md)通过 |
| `codebuddy-cli` | CodeBuddy | ACP v1 | `fine_grained` | 同 ACP 合同；`2.133.1` 只在 terminal `kind=fetch + query-only rawInput` 投影 Web 搜索，started `kind=other`、WebFetch 非 query 输入与 Grep `kind=search` 均不得准入 | terminal fetch positive、started/WebFetch/Grep negative fixture 通过 | Skill turn 与既有 Web wire 通过；[v1.52 本机文件矩阵](../versions/v1.52/runtime-acceptance.md)已用本机 MiniMax `minimax-m3` 完成五条真实 Run，新增仍为 path-only write |
| `qwen-code` | Qwen Code | ACP v1 | `fine_grained` | 同 ACP 合同 | 受控 fixture 通过 | Skill turn 通过；[`0.23.0` 文件操作矩阵](../versions/v1.52/runtime-acceptance.md)通过并记录 basename-only Diff 边界 |
| `trae-cn-cli` | TRAE CLI CN | ACP v1 | `fine_grained` | 同 ACP 合同；标准 `rawInput.command` 与 TRAE Bash 实测 `rawInput.Command` 的非空字符串进入公开 input，`Description` 等相邻字段保持私有；命令结构补全缺失的 execute kind；同 `toolCallId` 的 terminal 自带 command/kind/digest；Terminal content 只作 display anchor | 实测 `Command + Description` started、相邻 raw 字段排除、非 TRAE 大写字段 fail-closed、稀疏 terminal、非零 exit code 与固定 `printf` fixture 已建立 | `traecli 0.120.52` completion/cancel、Approval allow/deny、Missing-Send tool→final 与 MCP Projection 正式 Smoke 通过；[v1.52 文件操作矩阵](../versions/v1.52/runtime-acceptance.md)通过；完整 command-output 矩阵仍保留既有 nonzero status 漂移 |
...
| `deepseek-harness` | DeepSeek Harness | ACP v1 + official tools/result observer | `fine_grained` | 精确 Session/call 映射；bash/pwsh→execute、read/read_image→read、write/edit→对应文件语义、glob/grep→file_search、web_search→web_search、web_fetch→fetch、skill→tool；shell canonical exit/signal/timeout 决定结果，canonical path 决定 operation；write 接受 `before: string \| null` 与 string after，edit 只接受 string/string，并仅转换为标准 ACP terminal Diff；未知工具保留 other | bootstrap/工具身份/一次消费、write 显式 null、缺失 before、edit null 与大文件回退 fixture；复用 ACP 终态、Diff 和输出 owner | 0.1.5-rc.2 + MiniMax-M3 真实 read/add/edit/empty matrix 通过；add `+1/-0`、edit `+1/-1`、empty edit `+1/-0`；缺字段或不可信状态保持路径级回退；[新增文件增量证据](../research/deepseek-harness-runtime/acp-0.1.5-create-diff-evidence.json)不扩大平台资格 |
...
| ACP v1（`opencode-cli`、`copilot-cli`、`kiro-cli`、`qoder-cli`、`codebuddy-cli`、`qwen-code`、`trae-cn-cli`、`cursor-agent`、`kimi-code-cli`、`grok-build`、`deepseek-harness`） | ACP `session/update.tool_call_update`；私有 extension 与 run-level fallback 不补造 operation 或 Diff；DSH observer 只把官方完整文件状态修复为该标准 terminal shape | 成功 terminal native kind 精确 `read | edit | write`；read 要求同 ToolCall 唯一标准 location，write 沿用保守兼容路径准入；OpenCode 的 Boolean `rawOutput.metadata.exists` 仅在 metadata path 与同 ToolCall location/rawInput path 对齐时形成 add/update | terminal 累计 `content.type=diff { path, oldText?, newText }`；只接纳标准 ACP terminal Diff；DSH write 的显式 `before:null` 形成 add，edit 仍要求双侧字符串；缺字段、类型错误、超限或无变化时不注入 | read 显示“阅读”；明确 add 显示“新增”，其他 write 显示“编辑”；有 Diff 才显示计数并展开 |
...
Renderer 只消费 Canonical presentation/diffProjection，不含 Runtime 分支。Claude 的 Read/Write/Edit operation
只在 matching result 后发布；只有 Write 的 matching 结构化 update 结果可增加 update，只有 Edit 进入 exact
mutation，NotebookEdit 与 ApplyPatch 不准入该通道。
...
|---|---|---|---|---|
| `commandExecution` | 默认 `shell`；v3 typed read 为 `file` | 默认 `shell.execute`；v3 typed read 为 `file.read` | 无结构化名称时为空 | Core 不把 commandActions 翻译成标题；只有非空、全 read、唯一 path 的 actions 形成 typed read |
...
Codex 的 `cat`、`head`、`tail`、`sed` 没有命令名白名单。它们只有在 app-server 同一 item 报告的
`commandActions` 非空、每项 `type=read`、所有 path 相同且非空时才产生 schema 2 read；纯命令文本、复合
read/write action、多文件或缺 path 时仍为 Shell。started 和 completed 各自携带候选，同一 operation 按冻结的
classifier 归约；v1/v2 in-flight activity 不会中途切成 read。

### Command output durability audit

2026-08-26 对当时全部 13 个 Adapter 的归一化路径逐项核验后，terminal output 权威如下；Pi 后续成为第 14 个
Adapter，其 terminal output 由独立 JSONL 合同拥有：
...
| ACP v1 | identities：`opencode-cli`、`copilot-cli`、`kiro-cli`、`qoder-cli`、`codebuddy-cli`、`qwen-code`、`trae-cn-cli`、`cursor-agent`、`kimi-code-cli`、`grok-build`、`deepseek-harness` | terminal `tool_call_update` 归一为一条 `runtime.action.payload.output` | 这些 Adapter 不产生 `command.output.delta` |
...
文件操作 presentation 使用更窄的终态合同：只有成功 `completed`、累计 native kind 精确为
`read | edit | write` 且能证明唯一规范化路径时，才生成 schema 2 typed operation。read 只接纳同 ToolCall
标准 `locations[].path`；write 保留当前受控兼容来源。terminal 省略字段时可保留当前 Prompt 对相同 ToolCall 的
可信观察；冲突 kind 不覆盖首次可信 kind。路径不从 title/output 猜测，typed operation 也不自动生成
`diffProjection`。`read_file`、`write_file`、`apply_patch` 等普通分类仍可属于 file domain，但不因名称获得该
operation Evidence。
...
完整 assistant `tool_use` 名称精确为 `Read | Write | Edit` 且 `input.file_path` 非空时，Adapter 按 tool-use ID
暂存 schema 2 operation candidate；matching `tool_result` 明确非错误后才发布。Read 映射 read，Write/Edit 映射
write。Write 的结构化 `tool_use_result.type=update` 只有在 `filePath` 与 candidate path 完全一致时才映射
`changeKind=update`；字段缺失、create、未知类型、路径冲突与 Edit 不增加 changeKind。失败、缺失、取消、
Glob/Grep、NotebookEdit 与 ApplyPatch 不进入 typed operation。Claude Code 2.1.236 的真实 Write 验证了：不存在
文件与已有空文件同为 `create + originalFile=null`，已有非空文件为 `update`；因此 create 必须保守回退。
