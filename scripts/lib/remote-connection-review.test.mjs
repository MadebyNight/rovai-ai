...
test('remote connection design preserves production settings geometry, keyboard validation and reversible decisions', { timeout: 120_000 }, async t => {
  ...
  try {
    ...
    for (const theme of ['day', 'night']) {
      ...
      for (const [surface, state] of [['desktop', 'enabled'], ['desktop', 'off'], ['desktop', 'error'], ['desktop', 'loading'], ['web', 'enabled'], ['web', 'offline'], ['web', 'expired']]) {
        ...
        if (surface === 'desktop' && state === 'enabled') {
          ...
          for (const label of ['复制本机地址', '复制远程地址', '复制管理令牌']) {
            await browser.click(icon(label))
            await browser.wait(`${icon(label)}.title==='已复制'`)
            const expected = label === '复制管理令牌' ? `document.querySelector('#remote-token').value` : addressValue(label.slice(2))
          ...
          await browser.wait(`${icon('复制管理令牌')}.title==='复制管理令牌'`)
          await browser.click(icon('显示管理令牌'))
          assert.equal(await browser.evaluate(`document.querySelector('#remote-token').type`), 'text')
          assert.equal(await browser.evaluate(`${icon('隐藏管理令牌')}.getAttribute('aria-pressed')`), 'true')
          await browser.click(icon('隐藏管理令牌'))
    ...
    assert.equal(await browser.evaluate(`document.querySelector('.remote-addresses')===null && !document.querySelector('button[aria-label="复制管理令牌"]').disabled`), true, 'token is available before starting')
    ...
    await browser.click(icon('复制管理令牌'))
    assert.equal(await browser.evaluate(`${icon('复制管理令牌')}.title`), '复制管理令牌', 'failed copy must not show success')
