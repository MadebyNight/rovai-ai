...
test('phone workbench uses shared navigation, schedules and per-tab drafts', { timeout: 180_000 }, async t => {
  ...
  try {
    ...
    const imageResponses = browser.responses.slice(beforeImage)
    assert.equal(imageResponses.filter(response=>response.path==='/api/v1/uploads').length, 1)
    assert.ok(imageResponses.some(response=>response.path==='/api/v1/files'), 'Host confirms the exact source before a thumbnail appears')
    assert.equal(imageResponses.filter(response=>response.path==='/api/v1/files/bytes').length, 0, 'the original image is not downloaded again')
    await capture('local-image-preview')
    const beforeImageReload = browser.responses.length
    await browser.send('Page.reload')
    await browser.wait(`(()=>{const image=document.querySelector('.composer-attachment-card img');return image?.complete && image.naturalWidth===1})()`)
    assert.equal(browser.responses.slice(beforeImageReload).filter(response=>response.path==='/api/v1/files/bytes').length, 1, 'refresh falls back to authenticated Host bytes')
    ...
    await browser.wait(`[...document.querySelectorAll('.composer-attachment-card .attachment-open:not(:disabled)')].some(e=>e.getAttribute('aria-label')?.endsWith('mobile.html'))`)
    await browser.click(`[...document.querySelectorAll('.composer-attachment-card .attachment-open')].find(e=>e.getAttribute('aria-label')?.endsWith('mobile.html'))`)
...
test('phone execution shares Desktop evidence, wraps avatars and retains Run disclosures', { timeout: 120_000 }, async t => {
  ...
  try {
    ...
    for (const theme of ['day', 'night']) {
      ...
      const alignment = await browser.evaluate(`[...document.querySelectorAll('.execution-process-stage')].map(e=>{const node=e.querySelector('.execution-process-node').getBoundingClientRect();const row=e.querySelector('.execution-disclosure > summary').getBoundingClientRect();return Math.abs((node.top+node.bottom-row.top-row.bottom)/2)})`)
      assert.ok(alignment.every(delta=>delta<2), 'timeline nodes align with Run summary centers')
      const mobileRun = await browser.evaluate(`(()=>{const run=document.querySelector('.execution-process-stage.status-running');const summary=run.querySelector('.mobile-run-summary');return {time:summary.querySelector('time').textContent,desktopTime:run.querySelector('.execution-run-boundary time').textContent,label:summary.querySelector('.process-disclosure-label').textContent,desktopLabel:run.querySelector('.execution-run-boundary-state').textContent,current:summary.querySelector('.current-run-badge')?.textContent}})()`)
      assert.equal(mobileRun.time, mobileRun.desktopTime)
      assert.equal(mobileRun.label, mobileRun.desktopLabel)
      assert.equal(mobileRun.label, '执行中')
      assert.equal(mobileRun.current, '当前执行')
      ...
      const commands = await browser.evaluate(`[...document.querySelectorAll('.tool-group-items .tool-call-summary')].map(e=>e.getBoundingClientRect().toJSON())`)
      assert.ok(commands.length >= 2)
      assert.ok(commands.every(row => row.height === 32), 'expanded commands use the compact row height')
      assert.ok(commands[1].top - commands[0].bottom <= 4, 'expanded commands do not retain the previous gaps')
    ...
    assert.deepEqual(browser.errors, [])
    await writeFile(join(output, 'execution-validation.json'), JSON.stringify({ productionComponents: true, simulatedEvidence: true, realRuntime: false, widths: [360, 375, 390, 430, 844], themes: ['day', 'night'], checks: ['two-primary-tabs', 'two-entry-avatars-plus-overflow', 'reading-font-preference', 'reduced-motion', '32px-expanded-commands', '10-avatar-wrap', '3-runs', 'independent-disclosures-retained-across-member-switch', 'shared-command-group', 'rotation'] }, null, 2))
