...
test('phone workbench uses shared navigation, schedules and per-tab drafts', { timeout: 180_000 }, async t => {
  ...
  try {
    ...
    assert.equal(placeholder.font, placeholder.editorFont)
...
test('phone execution shares Desktop evidence, wraps avatars and retains Run disclosures', { timeout: 120_000 }, async t => {
  ...
  try {
    ...
    for (const theme of ['day', 'night']) {
      ...
      assert.equal(await browser.evaluate(`document.querySelectorAll('.mobile-camp-tabs .mobile-execution-running svg').length`), 1, 'phone shows an execution glyph for running members')
      ...
      assert.equal(await browser.evaluate(`document.querySelector('.mobile-camp-tabs .mobile-execution-running')===null`), true, 'waiting for approval does not claim a running member')
