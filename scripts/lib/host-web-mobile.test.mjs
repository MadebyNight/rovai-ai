...
test('phone workbench uses shared navigation, schedules and per-tab drafts', { timeout: 180_000 }, async t => {
  ...
  try {
    ...
    await capture('camp')
    await openSecondary('任务')
    await browser.click(byLabel('打开会话列表'))
    ...
    stage = 'settings'
    await click('设置')
    await browser.wait(`document.querySelector('.mobile-settings-search input')!==null`)
    assert.equal(await browser.evaluate(`document.querySelectorAll('.settings-sidebar-menu button').length`), 11, 'Desktop Host exposes all eleven settings categories')
    await fill('.mobile-settings-search input', '诊断')
    assert.deepEqual(await browser.evaluate(`[...document.querySelectorAll('.settings-sidebar-menu button')].map(e=>e.textContent.trim())`), ['诊断与修复'])
    await browser.evaluate(`(()=>{const input=document.querySelector('.mobile-settings-search input');const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;setter.call(input,'');input.dispatchEvent(new Event('input',{bubbles:true}))})()`)
    await browser.wait(`document.querySelectorAll('.settings-sidebar-menu button').length===11`)
    ...
    await browser.click(byLabel('返回设置'))
    assert.equal(await browser.evaluate(`[...document.querySelectorAll('.settings-sidebar-menu button')].some(e=>e.textContent.trim()==='远程连接')`), true, 'Desktop Host keeps remote connection settings available')
    for (const [label, section] of [['通用', 'general'], ['提醒', 'notifications'], ['Skills', 'skills'], ['MCP', 'mcp'], ['运行时', 'runtime'], ['远程连接', 'remote'], ['渠道', 'channels'], ['运行监控', 'monitoring'], ['诊断与修复', 'diagnostics']]) {
      await click(label)
...
test('standalone Server phone settings expose update controls, initial login and hide channels', { timeout: 60_000 }, async t => {
  ...
  try {
    ...
    assert.equal(await browser.evaluate(`[...document.querySelectorAll('.settings-sidebar-menu button')].some(e=>e.textContent.trim()==='远程连接')`), true, 'Server browsers can still manage their own connection and login')
    await browser.click(`[...document.querySelectorAll('.settings-sidebar-menu button')].find(e=>e.textContent.trim()==='远程连接')`)
