...
test('API adapter disables model tools, checks snapshot/finish and never falls back after refusal', async () => {
  ...
  try {
    for (const timeout of [undefined, null]) {
      const config = { ...(timeout === undefined ? {} : { timeoutMilliseconds: timeout }), snapshotId: 'fixed-test-snapshot', api: { endpoint: 'http://127.0.0.1/completions', keyEnvironmentVariable: keyName, modelVersionPolicy: 'pinned_snapshot' } }
      ...
      const adapter = createAdapter(config, { fetchImplementation: async (_url, init) => {
        if (timeout === null) assert.equal(init.signal, undefined)
        else assert.ok(init.signal instanceof AbortSignal)
      ...
      process.env[keyName] = 'local-test-value'
    }
