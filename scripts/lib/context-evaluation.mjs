...
export async function evaluatorDigest() {
  return digestJson({ qualification: await computeQualificationEvaluatorDigest(), files: await Promise.all(['scripts/lib/context-evaluation.mjs', 'scripts/lib/context-quality.mjs', 'packages/evaluation/src/report-html.ts', 'scripts/lib/context-judge-profile.mjs', 'scripts/lib/context-metric-contract.mjs', 'scripts/lib/context-regression-fixture.mjs', 'scripts/benchmark/execution/contract-test-evidence.mjs', 'scripts/benchmark/execution/current-contract-runner.mjs', 'scripts/benchmark/profiles/current-contract-conformance.mjs'].map(async path => [path, await digestFile(join(root, path))])) })
}
