...
export async function collectContractTestEvidence({ repositoryRoot, references, outputDirectory, timeoutMs }) {
  ...
  for (const [index, reference] of unique.entries()) {
    ...
    const execution = remaining <= 0 ? { code: null, stdout: '', stderr: '', timedOut: true, spawnError: null } : await execute('cargo', args, repositoryRoot, timeoutMs === null ? null : remaining)
...
function execute(command, args, cwd, timeoutMs) {
  return new Promise(resolve => {
    ...
    let killTimer
    const timer = timeoutMs === null ? undefined : setTimeout(() => { timedOut = true; terminate('SIGTERM'); killTimer = setTimeout(() => terminate('SIGKILL'), 5_000) }, timeoutMs)
