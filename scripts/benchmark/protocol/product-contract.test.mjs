...
test('Product Contract Fingerprint reads code/build authority and marks unavailable data explicitly', async () => {
  ...
  assert.equal(fingerprint.dataContractSchemaVersion.value, 112)
  assert.equal(fingerprint.campSnapshotSchemaVersion.value, 34)
  assert.equal(fingerprint.contextManifestVersion.value, 25)
  assert.equal(fingerprint.contextFormatterVersion.value, 25)
  assert.equal(fingerprint.contextDeliveryProfileVersion.value, 6)
  assert.equal(fingerprint.durableTaskContract.value.version, 3)
  assert.equal(fingerprint.builtInTransportVersion.value, 27)
