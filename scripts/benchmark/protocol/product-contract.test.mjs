...
test('Product Contract Fingerprint reads code/build authority and marks unavailable data explicitly', async () => {
  ...
  assert.equal(fingerprint.dataContractVersion.value, 'v1.59')
  assert.equal(fingerprint.dataContractSchemaVersion.value, 105)
  assert.equal(fingerprint.campSnapshotSchemaVersion.value, 34)
  assert.equal(fingerprint.contextManifestVersion.value, 23)
  assert.equal(fingerprint.contextFormatterVersion.value, 23)
  assert.equal(fingerprint.contextDeliveryProfileVersion.value, 5)
  assert.equal(fingerprint.durableTaskContract.value.version, 3)
  assert.equal(fingerprint.builtInTransportVersion.value, 24)
