...
test('Product Contract Fingerprint reads code/build authority and marks unavailable data explicitly', async () => {
  ...
  assert.equal(fingerprint.dataContractSchemaVersion.value, 107)
  assert.equal(fingerprint.campSnapshotSchemaVersion.value, 34)
  assert.equal(fingerprint.contextManifestVersion.value, 24)
  assert.equal(fingerprint.contextFormatterVersion.value, 24)
  ...
  assert.equal(fingerprint.builtInTransportVersion.value, 25)
