...
test('Product Contract Fingerprint reads code/build authority and marks unavailable data explicitly', async () => {
  ...
  assert.equal(fingerprint.dataContractVersion.value, 'v1.68')
  assert.equal(fingerprint.dataContractSchemaVersion.value, 122)
  assert.equal(fingerprint.campSnapshotSchemaVersion.value, 34)
  assert.equal(fingerprint.contextManifestVersion.value, 29)
  assert.equal(fingerprint.contextFormatterVersion.value, 29)
  assert.equal(fingerprint.contextDeliveryProfileVersion.value, 9)
  assert.equal(fingerprint.durableTaskContract.value.version, 5)
  assert.equal(fingerprint.builtInTransportVersion.value, 32)
