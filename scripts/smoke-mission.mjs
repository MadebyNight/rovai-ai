...
try {
  ...
  const follower = join(output, 'follower.mjs'), lead = join(output, 'lead.mjs'), continuation = join(output, 'continuation.mjs'), detached = join(output, 'detached.mjs'), nonGit = join(output, 'non-git.mjs')
  ...
  const failedCleanup = await waitForWorkspaceState(mission.missionId, 'cleanup_failed')
  assert.match(failedCleanup.diagnostic, /mission.workspace_branch_mismatch/)
  ...
  git(source, 'branch', workspace.branch, managedCommit)
  git(workspace.worktree_path, 'switch', workspace.branch)
  applied(await command('missions.workspace.cleanup', { missionId: mission.missionId }))
  ...
  report.cases.push('cleanup preserves a Worktree on a non-managed checkout; explicit recovery to the managed branch permits bounded cleanup and later deletion')
  report.runs = [...detachedResumed.agentRuns, ...plainSnapshot.agentRuns].map(({ id, status, workspace }) => ({ id, status, workspace }))
