...
try {
  ...
  await app.wait(`document.querySelector('.onboarding-primary')`)
  await app.click(`document.querySelector('.onboarding-primary')`)
  await app.wait(`document.querySelector('#onboarding-member-title')`)
  await app.click(`document.querySelector('.onboarding-primary')`)
  await app.wait(`document.querySelector('#onboarding-runtime-title')`)
  await app.evaluate('window.rovai.onboarding.deferRuntimeSetup()')
  await app.send('Page.reload')
  await app.wait(`document.querySelector('.unified-sidebar')`)
  ...
  await browser.wait(`document.querySelector('.web-login input')`)
  ...
  await browser.wait(`document.querySelector('.unified-sidebar') && !document.querySelector('.web-login-overlay')`)
  const settings = async (surface, section) => {
    ...
    await surface.wait(`document.querySelector('.settings-sidebar-menu')`)
  ...
  await browser.wait(`document.querySelector('input[aria-label="一键创建新对话"]')?.checked === true`)
  ...
  await browser.wait(`document.querySelector('input[aria-label="一键创建新对话"]')?.checked === false`)
  ...
  await browser.wait(`document.querySelector('.general-save-row button')?.disabled === true`)
  ...
  await app.wait(`document.querySelectorAll('.general-default-member input:checked').length === 2`)
  ...
  for (const theme of ['day', 'night']) {
    ...
    await browser.wait(`document.querySelector('input[value="${theme}"]')`)
    await browser.click(`document.querySelector('input[value="${theme}"]').closest('label')`)
    await browser.wait(`document.documentElement.dataset.theme === '${theme}'`)
    await browser.click(`document.querySelector('.settings-sidebar-back')`)
    await browser.wait(`document.querySelector('button[aria-label="新对话"]')`)
    await browser.click(`document.querySelector('button[aria-label="新对话"]')`)
    await browser.wait(`document.querySelector('.new-camp-dialog .compact-primary:not(:disabled)')`)
    ...
    await browser.wait(`document.querySelector('.new-camp-member-grid')`)
  ...
  await browser.wait(`window.__createdPreferenceCamps.length===1 && !document.querySelector('.new-camp-dialog') && document.querySelector('#camp-message')`)
  assert.equal((await app.evaluate('window.rovai.generalPreferences.get()')).oneClickNewConversationEnabled, true)
  await browser.click(`document.querySelector('button[aria-label="新对话"]')`)
  await browser.wait(`window.__createdPreferenceCamps.length===2 && !document.querySelector('.new-camp-dialog')`)
  const created = await browser.evaluate('window.__createdPreferenceCamps')
  const pending = await app.evaluate(`window.rovai.request('camp.open',{campId:${JSON.stringify(created[1])}})`)
  ...
  await browser.wait(`performance.timeOrigin!==${timeOrigin} && document.querySelector('input[aria-label="一键创建新对话"]')?.checked === true`)
