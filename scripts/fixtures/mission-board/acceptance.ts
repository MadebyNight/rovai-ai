export async function runMissionAcceptance(): Promise<{ ok: true; cases: string[] }> {
  ...
  check(qa.calls.filter((call:any) => call.method === 'missions.changes').length === changeReads + 1 && qa.missionChangesInFlight() === 1 && qa.missionChangesMaxInFlight() === 1, 'Manual, Run-completion, focus and visibility refreshes keep one workspace read in flight')
  await until(() => qa.calls.filter((call:any) => call.method === 'missions.changes').length === changeReads + 2, 'Queued workspace invalidations launch one follow-up read')
...
export async function runMissionCheckoutViewAcceptance(): Promise<{ ok: true; cases: string[] }> {
  ...
  window.dispatchEvent(new Event('focus')); document.dispatchEvent(new Event('visibilitychange'))
  await new Promise(resolve => setTimeout(resolve, 150))
  check(qa.calls.filter((call: any) => call.method === 'missions.changes').length === changeReads + 1 && qa.missionChangesInFlight() === 1 && qa.missionChangesMaxInFlight() === 1, 'Refresh triggers do not overlap the delayed workspace read')
  await until(() => qa.calls.filter((call: any) => call.method === 'missions.changes').length === changeReads + 2, 'Refresh triggers produce one follow-up workspace read')
  ...
  await new Promise(resolve => setTimeout(resolve, 180))
  check(qa.calls.filter((call: any) => call.method === 'missions.changes').length === readsBeforeClose + 1, 'Unmounting drops a queued follow-up workspace refresh')
  check(qa.errors.length === 0, qa.errors.join('\n'))
