...
].map(([title,status,tags],i)=>({missionId:`mission-${i}`,campId:`rvcamp_01h47kvsy5fk1shh6w1g60eec${i}`,title:title as string,description:'让使命从保存、开始、恢复到交付都有清晰的状态。复用现有会话组件，并验证工作目录、草稿和文件预览。\n这段描述用于验证完整描述展开后的布局。',status:status as any,tags:tags as string[],projectPath:'/workspace/rovai-ai',projectBindingKind:'directory',sourceBranch:'HEAD',sourceMessageId:null,createdAt:now,updatedAt:new Date(Date.now()-86400000).toISOString(),memberAgentIds:profiles.map(a=>a.agentId),defaultLeadAgentId:profiles[0].agentId,runningAgentIds:[],hasUnread:i===0}))
...
const client={...model.client,onInvalidated:undefined,onEvent:(fn:any)=>{events.add(fn);return()=>events.delete(fn)},request:async(method:string,p:any={})=>{
 ...
 if(method==='missions.update'){Object.assign(m!,c,{updatedAt:new Date().toISOString()});changed();return applied({missionId:m!.missionId,changed:true})}
 ...
 if(method==='missions.delivery')return {campId:m!.campId,workingDirectory:'/workspace/rovai-ai-mission-'+m!.missionId,git:true,workspace:{id:'workspace',workingDirectory:'/workspace/rovai-ai-mission-'+m!.missionId,worktreePath:'/workspace/rovai-ai-mission-'+m!.missionId,branch:'rovai/mission/'+m!.missionId,baseSha:'a'.repeat(40),state:'ready',diagnostic:null},pullRequests:[],files:[{attachmentId:'review-attachment',displayName:'interaction-review.md',kind:'file',fileCount:1,mediaType:'text/markdown',byteSize:1024,previewKind:'none',messageId:snapshot(m!).messages[1].id,agentId:profiles[0].agentId,createdAt:now}]}
